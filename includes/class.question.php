...
class question extends connection
{
	...
	function __construct($question_id=""){
		...
		parent::__construct();
		$q="select *from questions where question=:question";
		$stmt=$this->con->prepare($q);
		$stmt->bindParam(":question",$this->question_id);
		$stmt->execute();
	...
	public function create($question,$option_1,$option_2,$option_3,$option_4,$correct_answer,$quiz_id,$correct_answer,$created_by,$qtype="CHOICE_BASED_EXAM",$diagrams=""){
		$error="";

		$qx=new question($question);
		if ($qx->question_exists >0) {
			$error="failed! question already exists.";
		}else{
		$q="insert into questions(question,option_1,option_2,option_3,option_4,correct_answer,quiz_id,diagrams,created_by,qtype) values(:question,:option_1,:option_2,:option_3,:option_4,:correct_answer,:quiz_id,:diagrams,:created_by,:qtype)";
		$stmt=$this->con->prepare($q);
...
$q="update questions set question=:question,option_1=:option_1,option_2=:option_2,option_3=:option_3,option_4=:option_4,correct_answer=:correct_answer,diagrams=:diagrams where question_id=:question_id";
		$stmt=$this->con->prepare($q);
		$stmt->bindParam(":question",$question);
		$stmt->bindParam(":option_1",$option_1);
		$stmt->bindParam(":option_2",$option_2);
		...
		$stmt->bindParam(":correct_answer",$correct_answer);
		$stmt->bindParam(":diagrams",$diagrams);
		$stmt->bindParam(":question_id",$this->question_id,PDO::PARAM_STR);
		$stmt->execute();
		$error="success";
	}
	return $error;
